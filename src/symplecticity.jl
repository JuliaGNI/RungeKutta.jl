...
function check_symplecticity(tab::Tableau{T}; atol = 16eps(T), rtol = 16eps(T)) where {T}
    a, b = tab.a, tab.b
    [isapprox(b[i] * a[i, j] + b[j] * a[j, i], b[i] * b[j]; atol = atol, rtol = rtol)
...
"""
function SymplecticTableau(tab::Tableau{TT}; name = Symbol("Symplectic", tab.name),
        T = TT, R∞ = tab.R∞) where {TT}
...
function SymplecticConjugateTableau(tab::Tableau{TT}; name = tab.name, T = TT, R∞ = tab.R∞) where {TT}
    Tableau{T}(
        name, tab.s, symplectic_conjugate_coefficients(tab.a, tab.b), tab.b, tab.c; R∞ = R∞)
end
...
"""
function SymplecticPartitionedTableau(
        tab::Tableau{TT}; name = Symbol("Symplectic", tab.name),
        T = TT, R∞ = tab.R∞) where {TT}
    PartitionedTableau{T}(name, tab, SymplecticConjugateTableau(tab); R∞ = R∞)
end
